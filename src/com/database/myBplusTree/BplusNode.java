...
public class BplusNode {
     ...
     * 构造函数
     */
    public BplusNode(Pager pager, byte type, TableSchema schema, String sql) {
    ...
    public BplusNode(Pager pager, Page page, TableSchema schema, String sql) {
        /* 从page中读取内容，构造一个节点 */
        this.pager = pager;
        this.sql = sql;
        this.page = page;
    ...
    public void loadChildren(BplusNode node){
        ...
        for(int i=0;i<node.entries.size();i++){
            ...
            if(page.getPageType()==3){
                tempSchema = TableSchema.buildTableSchema(sql);
            }else{
                tempSchema = TableSchema.getTreeInternalSchema();
            }
            BplusNode temp = new BplusNode(pager, page, tempSchema, sql);
    ...
    public void insertOrUpdate(Integer key, String value, BplusTree tree){
        ...
        if (page.getPageType() == PageType.TABLE_LEAF){
             ...
             */
            BplusNode left = new BplusNode(pager, PageType.TABLE_LEAF, schema, sql);
            BplusNode right = new BplusNode(pager, PageType.TABLE_LEAF, schema, sql);

            if (page.getpPrev() != 0){
                if(previous == null)
                    previous = new BplusNode(pager,pager.aquirePage(page.getpPrev()), schema, sql);
            ...
            if (page.getpNext() != 0) {
                next = new BplusNode(pager, pager.aquirePage(page.getpNext()),schema, sql);
            ...
            if (page.getpParent() != 0) {
                //调整父子节点关系
                if(parent == null){
                    parent = new BplusNode(pager,pager.aquirePage(page.getpParent()),schema, sql);
                    loadChildren(parent);
                }
...
//        flushPage(right.entries,right);     /* 刷新右节点页面数据域 */
    ...
    protected void updateInsert(BplusTree tree){
        ...
        if (entries.size() > tree.getOrder()) {
            //分裂成左右两个节点
            BplusNode left = new BplusNode(pager,PageType.TABLE_INTERNAL,schema, sql);
            BplusNode right = new BplusNode(pager,PageType.TABLE_INTERNAL,schema, sql);
            ...
            if (page.getpParent() != 0) {
                ...
                BplusNode parent = new BplusNode(pager, p,schema, sql);
                left.page.setpParent(page.getpParent());
                right.page.setpParent(page.getpParent());
    ...
    protected void updateRemove(BplusTree tree) {
        ...
        if (entries.size() < tree.getOrder() / 2 || entries.size() < 2) {
            if (page.getPageType() == PageType.TABLE_ROOT) {
                ...
                BplusNode root = new BplusNode(pager, p,schema, sql);
                tree.setRoot(root);
                tree.setHeight(tree.getHeight() - 1);
            ...
            BplusNode previous = new BplusNode(pager, pager.aquirePage(page.getpPrev()),schema, sql);
            if (page.getpPrev() != 0
                ...
                BplusNode borrow = new BplusNode(pager, pager.aquirePage(pgno),schema, sql);
                borrow.page.setpParent(page.getPgno());

                BplusNode parent = new BplusNode(pager, pager.aquirePage(page.getpParent()),schema, sql);
            ...
            BplusNode next = new BplusNode(pager, pager.aquirePage(page.getpNext()),schema, sql);
            if (page.getpNext() != 0
...
//                    previous.children.get(i).parent = this;
            ...
            if (page.getpNext() != 0
                ...
                for (int i = 0; i < next.entries.size(); i++) {
                    ...
                    BplusNode child = new BplusNode(pager, pager.aquirePage(pgno),schema, sql);
//                    entries.add(child.entries.get(0).getValue(), child.page.getPgno());
    ...
    public String remove(Integer key, BplusTree tree){
        ...
        if (page.getPageType() == PageType.TABLE_LEAF){
            ...
            if (previous != null
                ...
                if (previous.page.getpPrev() != 0) {
                    ...
                    new BplusNode(pager, pager.aquirePage(temp.page.getpPrev()),schema, sql).page.setpNext(page.getPgno());
            ...
            if(next != null
                ...
                if (next.page.getpNext() != 0) {
                    BplusNode temp = next;
                    new BplusNode(pager, pager.aquirePage(temp.page.getpNext()),schema, sql).page.setpPrev(page.getPgno());
        ...
        if (key.compareTo(entries.get(0).getKey()) < 0) {
            ...
            children = new BplusNode(pager, pager.aquirePage(pgno),schema, sql);
